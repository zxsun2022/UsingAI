...
- **决策聚焦解读** — 默认围绕一个有截止时间的具体事项，只分析相关宫位与时间层级
- **完整盘面数据** — 可只排盘不解读；明确要求完整解读时再遍历 12 宫
- **运势分析** — 查看流年、流月、流日运势
...
> 若目标工具把 Skill 挂载为只读目录且未预装依赖，请先把整个 `ziwei-iztro-reader` 目录复制到临时可写目录，再在副本的 `scripts/` 中运行 `npm install`。
...
1. **通过门禁** — 解读需先明确具体待决事项和截止时间；仅排盘无需决策信息
2. **自动排盘** — 调用 [iztro](https://github.com/SylarLong/iztro) 库生成紫微斗数命盘
3. **结构化分析** — 保留完整 12 宫数据，默认筛选与决策相关的宫位与层级
4. **解读输出** — 默认 focused；只有明确要求完整解读时使用 full
