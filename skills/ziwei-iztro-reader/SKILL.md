...
description: 使用 iztro 生成紫微斗数盘面数据，并围绕有截止时间的具体待决事项输出聚焦式分层解读。适用于用户要求基于出生日期、出生时辰、出生地与性别排盘，或要求本命、流年、流月、流日及未来时间段分析时。允许仅排盘不解读；解读默认 focused，只有用户明确要求完整解读时才使用 full。
---
