...
### ~~AI 解读时自行推算日期导致错误~~（v0.3.0 已修复）

- 之前 AI 在解读时可能自行推算星期几、农历日期、流月天干等，容易出错。
- 现在脚本输出中增加了 `dayOfWeek`、`dateSummary`（含 `monthlyHeavenlyStem`、`dailyHeavenlyStem` 等）字段。
- SKILL.md 中增加了"日期处理守卫规则"章节，强制 AI 从输出中读取而非自行计算。
...
### 3. iztro 流月天干与传统"五虎遁"口诀可能存在口径差异

- 现状：iztro 库内部的流月天干排法与部分传统紫微斗数教材中的"五虎遁年起月法"可能给出不同结果
- 影响：如果用户拿 iztro 的流月四化与其他软件/手工排盘对比，可能发现差异
- 当前处理：以 iztro 输出为准，在 SKILL.md 中明确了"严禁自行推算，以脚本输出为准"的规则
- 优先级：低（属于门派/算法口径差异，非 bug）
